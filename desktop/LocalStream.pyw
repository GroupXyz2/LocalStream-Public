...
class MusicPlayer(QMainWindow):
    ...
    def import_playlist_from_folder(self):
        ...
        replace_playlist = False
        
        if name in self.playlists:
            reply = QMessageBox.question(self, "Playlist Exists", 
                                        f"A playlist named '{name}' already exists. Replace it or add missing songs?",
            ...
            if reply == QMessageBox.StandardButton.Cancel:
                ...
                for song in self.playlists[name]["songs"]:
                    try:
                        ...
                        if file_path.exists():
                            if self.strict_deduplication:
                                existing_signatures.add(file_path.name)
                            else:
                                file_stat = file_path.stat()
                                existing_signatures.add((file_path.name, file_stat.st_size))
                        else:
                            existing_signatures.add(song["path"])
                    except Exception:
                        existing_signatures.add(song["path"])
                ...
                if existing_signatures:
                    try:
                        if file.exists():
                            ...
                            if file_signature in existing_signatures:
                                skipped_count += 1
                                continue
                        else:
                            if file_path_str in existing_signatures:
                                skipped_count += 1
                                continue
                    except Exception:
                        if file_path_str in existing_signatures:
        ...
        if not songs:
            if skipped_count > 0:
                QMessageBox.information(self, "No New Songs", 
                                      f"All {skipped_count} songs from the folder are already in the playlist.")
            else:
        ...
        if replace_playlist or name not in self.playlists:
            self.playlists[name] = {
                "songs": songs,
                "created": "imported",
                "persistent": False
            ...
            self.playlists[name]["songs"].extend(songs)
        
        self.save_playlists()
        self.refresh_playlist_sidebar()
        
        message = f"Imported {len(songs)} songs into playlist '{name}'!"
        if skipped_count > 0:
            message += f"\n{skipped_count} duplicate songs were skipped."
    ...
    def show_playlist_context_menu(self, position: QPoint):
        ...
        import_folder_action = QAction("Import Missing from Folder...", self)
        import_folder_action.triggered.connect(lambda: self.import_missing_from_folder(playlist_name))
    ...
    def import_missing_from_folder(self, playlist_name):
        """Import all audio files from a folder that aren't already in the playlist"""
        folder = QFileDialog.getExistingDirectory(
            self,
            "Select Folder to Import Missing Files From",
            str(self.music_folder),
        ...
        if not audio_files:
            QMessageBox.information(self, "Import Missing Files", "No audio files found in the selected folder.")
            return
            ...
            try:
                ...
                if file_path.suffix.lower() == '.mp3':
                    try:
                        ...
                        error_files.append(f"{file_path.name}: Not a valid MP3 file - {e}")
                        continue
                elif file_path.suffix.lower() == '.mp4':
                    try:
                        audio = MP4(file_path_str)
                    except Exception as e:
                        error_files.append(f"{file_path.name}: Not a valid MP4 file - {e}")
                        continue
                else:
                    continue
                ...
                if file_path.suffix.lower() == '.mp3' and hasattr(audio, 'tags') and audio.tags:
                    ...
                    for tag in audio.tags.values():
                        if isinstance(tag, APIC):
                            album_art = tag.data
                            break
                elif file_path.suffix.lower() == '.mp4' and hasattr(audio, 'tags') and audio.tags:
                    if '\xa9nam' in audio.tags:
                        title = str(audio.tags['\xa9nam'][0])
                    if '\xa9ART' in audio.tags:
                        artist = str(audio.tags['\xa9ART'][0])
                ...
                error_files.append(error_msg)
                print(f"Error importing {file_path_str}: {e}")
        ...
        message = f"Import complete!\n\nAdded: {added_count}\nAlready in playlist: {duplicate_count}\nTotal files scanned: {len(audio_files)}"
        ...
        QMessageBox.information(self, "Import Missing Files", message)
